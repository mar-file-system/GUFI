...
```bash
gufi_spread_config.sh       \
    path/to/fsid.sql        \
    path/to/fsid.db         \
    path/to/extdbprefix.sql \
    path/to/extdbprefix.db
```
...
- `copy_to_shard`: A program that copies external data to a SQLite 3
  shard database for a single filesystem entry.
    - This program must take in 4 arguments:
        - shard path
        - entry path
        - entry inode
        - entry mtime
    ...
    - If not storing different versions of the extracted data,
      remember to delete the old external database data.
    - The schema of these SQLite 3 databases do not have to be the
      same as the final external databases, but it is recommended that
      they match to keep things easy.
    - See [`examples/spread_tree/copy_to_shard.sh`](/examples/spread_tree/copy_to_shard.sh)

- `extdb_schema`: A SQL file containing the schema of external
  databases for this prefix and fsid. At least one table requires an
  `inode` (or equivalent) column so that the filesystem metadata can
  join against it. A `mtime` column is also recommended.
    - This SQL is run the first time it is needed in a directory, not
    repeatedly at every entry that requires an external database.
    - See [`examples/spread_tree/schema.sql`](/examples/spread_tree/schema.sql)

- `shard_to_extdb`: A SQL file containing one or more queries to copy
  data from the shard to the index. This was done to allow for caller
  defined transformations of the data (such as selecting subsets of
  the shard to be copied) instead of having the workflow run `SELECT
  *` on a list of table names.
    - If not storing different versions of the extracted data in an
      external database, remember to delete the old data for the
      current entry.
    - See [`examples/spread_tree/shard_to_extdb.sql`](/examples/spread_tree/shard_to_extdb.sql)

- `gen_shard_path`: An optional shard path generator may also be
  provided. If one is not, when inserting into `extdbprefix.db`, pass
  in `NULL` as its value.
...
(
    ...
    echo "VALUES ("
    echo "        'prefix', 'spread_tree', NULL,"
    echo "        'examples/spread_tree/copy_to_shard.sh',"
    echo "        'examples/spread_tree/schema.sql',"
    echo "        'examples/spread_tree/shard_to_extdb.sql'"
    echo ");"
) | sqlite3 path/to/extdbprefix.db
```
...
```bash
gufi_spread_tree.sh path/to/fsid.db path/to/extdbprefix.db fsid prefix
```
...
```bash
gufi_spread_to_extdb.sh path/to/fsid.db path/to/extdbprefix.db fsid prefix
```
...
```bash
gufi_query \
    -I "CREATE TABLE external_data(inode TEXT, ...);" \
